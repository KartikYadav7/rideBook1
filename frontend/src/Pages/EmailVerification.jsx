      ...
      {otp.map((value, index) => {
        return (
          <input
            ...
            className="w-10 h-12  border border-blue-200 rounded-lg px-4 py-2 bg-blue-50 focus:outline-none focus:ring-2 focus:ring-primary text-center text-base"
          />
        );
      })}
    </div>
...
const EmailVerification = () => {
  ...
  const handleOtpSubmit = async () => {
    try {
      ...
      const res = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/verifyCode`, {
        userId,
        code: otp,
      });
      if (res.data.success) {
